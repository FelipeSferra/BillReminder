<script>
    var monthBills = {{ Js::from($monthBillsByType) }}
    ...
    if (monthBills.length === 0) {
        ...
        var data = {
            labels: monthBills.map(item => item.DESCRICAO),
            datasets: [{
                backgroundColor: monthBills.map(function(item) {
                    return 'rgba(' + hexToRgb(item.ID_HEX) + ', 0.2)';
                }),
                borderColor: monthBills.map(function(item) {
                ...
                borderWidth: 1,
                data: monthBills.map(item => item.TOTAL_VALOR),
            }]
        };
